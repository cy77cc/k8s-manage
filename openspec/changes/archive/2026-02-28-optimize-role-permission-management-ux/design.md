...
当前治理模块已从系统设置中独立到右侧菜单，但仍存在操作可用性缺口：角色管理页以“点击表格行打开详情/编辑抽屉”为主交互，权限编辑入口不够显式；用户管理当前缺少可编辑能力；权限管理页授权依赖逐项点击，在权限数量增长后操作负担高。
...
- 在用户管理页补齐“编辑用户信息/角色绑定”等核心操作，形成可闭环治理能力。
- 在权限编辑中支持批量勾选、批量取消、分组操作与搜索过滤，提升大规模权限集编辑效率。
- 补齐角色管理与权限管理关键动作链路，并统一交互反馈与确认模型。
...
1. 前端先落地用户/角色页显式操作入口与新权限编辑交互（保持兼容旧接口）。
2. 若发现旧接口能力不足，再补充用户编辑和权限批量更新接口并双端联调。
3. 灰度验证治理角色（超级管理员、平台管理员）主流程，观察误操作率与操作时长。
4. 回滚策略：保留旧组件与旧提交路径，出现严重问题可通过前端开关回退。

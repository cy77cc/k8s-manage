...
The command bridge MUST support selecting the multi-domain orchestration path through `ai.use_multi_domain_arch`. The bridge MUST also support selecting turn/block streaming rollout through `ai.use_turn_block_streaming`. Both toggles SHALL default to `false` and SHALL preserve legacy-compatible behavior as fallback.
...
#### Scenario: turn-block streaming rollout is enabled
...
- **AND** disabling the toggle MUST preserve the current compatibility-first chat behavior

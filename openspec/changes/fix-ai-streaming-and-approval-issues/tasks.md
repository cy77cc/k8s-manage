...
- [ ] 1.1 修改 `internal/ai/orchestrator.go` 的 `streamExecution` 函数
...
- [ ] 2.1 修改 `internal/ai/runtime/sse_converter.go`
...
- [ ] 3.1 检查 `internal/service/ai/handler.go` 中 chatRecorder 初始化
  - 确保 chatStore 正确传递
  - 添加日志记录

- [ ] 3.2 修改 `internal/service/ai/session_recorder.go`
...
- [ ] 4.1 检查 `internal/ai/tools/tools.go` 中工具包装逻辑
  - 添加日志记录工具包装过程
  - 验证工具名称与 registry 匹配
...
- [ ] 5.1 修改 `internal/ai/orchestrator.go`
  - 检查内容是否是工具参数 JSON
  - 过滤掉 `{"steps": [...]}` 格式的输出
